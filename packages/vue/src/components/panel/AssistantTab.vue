...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <div class="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4">
      ...
      <div v-if="chat.thread.length === 0" class="flex items-start gap-2 text-[13px] leading-relaxed text-muted">
      ...
      <div v-for="(m, i) in chat.thread" :key="i" class="flex"
        :class="m.from === 'me' ? 'justify-end' : 'justify-start'">
        <p :data-test="m.from === 'ai' ? 'ai-answer' : undefined"
          class="max-w-[82%] rounded-lg px-3 py-2 text-[13px] leading-relaxed whitespace-pre-wrap" :class="m.from === 'me'
            ? 'rounded-br-sm bg-accent text-accent-ink'
            : 'ai-bubble-border rounded-bl-sm text-text'
            ">
          <Icon v-if="m.from === 'ai'" :icon="Sparkles" :size="13" class="mb-0.5 inline text-ai-2" />
          <template v-for="(seg, si) in segments(m.text)" :key="si">
            <template v-if="seg.kind === 'refs'">
              <template v-if="seg.refs.some((r) => m.sources[r])">
                <CitationChip v-for="r in seg.refs.filter((ref) => m.sources[ref])" :key="r" :source="m.sources[r]!"
                  class="mr-1" />
              </template>
          ...
          <span v-if="m.from === 'ai' && m.text === '' && chat.status === 'streaming'" class="text-faint">…</span>
        </p>
      </div>
    </div>
    ...
    <div class="border-t border-line p-3">
      ...
      <form v-if="settings.flags.chatbotEnabled"
        class="flex items-center gap-2 rounded-lg border border-line-strong bg-sunken px-3 py-2 focus-within:border-accent"
        @submit.prevent="onSubmit">
        <input v-model="draft" type="text" data-test="ai-input" placeholder="Ask about your notifications…"
          aria-label="Ask the assistant" :disabled="chat.status === 'streaming'"
          class="flex-1 bg-transparent text-[13px] text-text placeholder:text-faint focus:outline-none disabled:cursor-not-allowed"
          @keydown.enter.prevent="onSubmit" />
        <button type="submit" data-test="ai-send" aria-label="Send"
      ...
      <p v-else data-test="ai-off" class="rounded-lg border border-line bg-sunken px-3 py-2 text-[13px] text-muted">
