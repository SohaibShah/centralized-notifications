...
| `read`     | `"true"` \| `"false"` (string) | no       | **Only meaningful together with [`group`](#grouping).** Scopes a group drill-in to a single read-state: `"false"` = the group's **unread** members, `"true"` = its **read** members. Omitted = the whole subject (both read-states). This exists because [grouped stacks are split by read-state](#grouped-read-state-split) — a stack's drill-in shows only that stack's own members. The grouped drill-in cursor is scoped to `read` too (see the [cursor note](#request)).                                                                                      |
| `priority` | string (CSV, ≤ 200 chars)      | no       | **Grouped-only** ([`grouped=true`](#request)) structured filter. A comma-separated list of [`priority`](#schema) values, e.g. `priority=critical,high`; each part must be one of `low`/`normal`/`high`/`critical` (any other value → `400 invalid query parameters`), **max 4** values. **Any-of** — a member matching **any** listed priority counts. Applied to a group's members **before aggregation** (see [grouped structured filters](#grouped-structured-filters)); **ignored** on the flat feed and on a `group=` drill-in. Omitted = no priority filter. |
| `module`   | string (CSV, ≤ 300 chars)      | no       | **Grouped-only** ([`grouped=true`](#request)) structured filter. A comma-separated list of module ids, e.g. `module=dsr,billing`; free-form strings, **max 20** values, each **≤ 100 chars** (over the limits → `400 invalid query parameters`). **Any-of** across the listed modules. Applied to a group's members **before aggregation** (see [grouped structured filters](#grouped-structured-filters)); **ignored** on the flat feed and on a `group=` drill-in. Omitted = no module filter.                                                                   |
...
**Sort-, view-, group-, and read-scoped cursor.** `cursor` is an opaque base64url token encoding the last returned row's ordering key **and the `sort`, `view`, `group`, and `read` it was issued under**; clients must treat it as opaque. The keyset predicate is sort-specific, the `view` selects a different row set, the `group` filter narrows to one stack's members, and the [`read`](#request) filter narrows a drill-in to one read-state, so a cursor is only valid when replayed under the same `sort`, `view`, `group`, **and** `read` — passing a cursor issued under one `sort`/`view`/`group`/`read` with a different value (e.g. a drill-in cursor issued under `read=false` replayed under `read=true`, or one `group` replayed under another) returns `400 { "error": "invalid cursor" }`, the same response as a malformed/undecodable cursor. In normal use this never happens: when the user changes sort, view, or drills into a group, the client refetches page 1 (no cursor) rather than reusing the previous page's cursor. The [grouped feed](#grouping) (`grouped=true`) uses its **own** cursor, valid only for the grouped read and **sort-scoped** the same way (a grouped cursor issued under one `sort` is rejected under another).
...
- **`grouped=true` — the collapsed grouped feed.** Returns one entry per stack (or standalone notification) instead of the flat list, keyset-paginated. Audience-scoped and mute-filtered exactly like the flat feed. **Honors the same [`sort`](#request) param as the flat feed** (`newest` | `oldest` | `priority-high` | `priority-low`, default `newest`) — see [ordering](#grouped-ordering) below. The response is a **`GroupedPage`** (not the flat `{ items, nextCursor }`):
  ...
  **Ordering.** The `sort` param orders the entries the same way it orders the flat feed. Under `newest`/`oldest`, entries are ordered by their representative's `createdAt` (the group's latest member). Under `priority-high`/`priority-low`, each entry is ranked by its **most-severe member** (`topPriority`), newest-first within a priority level. Default is `newest`.
  ...
  <a id="grouped-structured-filters"></a>
  **Structured filters ([`priority`](#request) / [`module`](#request)).** The grouped feed accepts two **optional, grouped-only** filters — [`priority`](#request) (CSV of `low`/`normal`/`high`/`critical`, any-of, max 4) and [`module`](#request) (CSV of module ids, any-of, max 20). They **only ever narrow** results and **compose with** the existing [audience scoping](#audience-scoping) and [mute filter](#enforcement--where-mutesnooze-rules-take-effect); they never re-add a row those exclude. The two are combined with **AND** (a member must match the priority filter **and** the module filter when both are given), while values **within** each filter are **any-of**.

  The key behavior is **where** they apply: each filter narrows a group's **members before aggregation**, so every per-entry field recomputes over **only the matching members** — `groupTotal` counts only matching members, `topPriority` is the most-severe priority **among the matching members**, and the representative (the `id`/`title`/`createdAt`/etc.) is the most-recent **matching** member. A subject with **no matching members drops out of the page entirely** (it is not returned as an empty or zero-count entry). Example: a `groupKey` with 4 members (2 `critical`, 2 `normal`) under `priority=critical` returns one entry with `groupTotal: 2` and `topPriority: critical`; the same subject under `priority=low` is omitted.

  These filters apply to the **grouped stacks list only**. They are **ignored** on the flat feed and on the [`group=` "See all" drill-in](#request) — a drill-in always returns the whole thread regardless of any `priority`/`module` params on the request.

  ```
  GET /notifications?grouped=true&priority=critical,high&module=dsr
  ```

  Returns the grouped stacks, but with each stack's `groupTotal`/`topPriority`/representative computed over only its `critical`-or-`high` members from the `dsr` module, and any stack with no such member omitted.

- **`group=<key>` — the "See all" drill-in.** Restricts the (flat) feed to a single group's members, identified by the group's `groupKey`. Reuses the normal flat feed shape (`{ items, nextCursor }`) and the same ordering and pagination; the cursor is additionally scoped to the `group` filter, and to the [`read`](#request) read-state filter when one is passed (see the [cursor note](#request) above). Pass [`read=true`](#request)/`read=false` alongside `group` to restrict the drill-in to that stack's read or unread members (omitted = both) — the read-state a grouped stack was split under.

Feed items also now always carry two **optional** grouping fields, `groupKey` (string) and `groupLabel` (string) — both present when the notification belongs to a stack, and **both absent when it is standalone (ungrouped)**. `groupKey` is the value to pass back as `?group=` to drill into the stack.
...
| `groupKey`   | string (optional) | The stack this notification belongs to. **Absent when the notification is standalone (ungrouped).** Pass it back as [`?group=`](#request) to drill into the stack's members.                                                                                 |
| `groupLabel` | string (optional) | Human-readable label for the stack. Present/absent together with `groupKey`.                                                                                                                                                                                 |
...
| Status | Body                                                      | Reason                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------ | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `400`  | `{ "error": "invalid query parameters" }`                 | `limit` out of range (`< 1` or `> 100`) or non-numeric, `sort` not one of `newest`/`oldest`/`priority-high`/`priority-low`, `view` not one of `active`/`muted`, `grouped` not one of `"true"`/`"false"`, `priority` containing a value not in `low`/`normal`/`high`/`critical` or more than 4 values, or `module` with more than 20 values / a value over 100 chars (or the raw `priority`/`module` string over 200/300 chars). |
| `400`  | `{ "error": "grouped and group are mutually exclusive" }` | `grouped=true` was passed together with a `group=<key>` value — pick one grouping mode, not both.                                                                                                                                                                                                                                                                                                                               |
| `400`  | `{ "error": "invalid cursor" }`                           | `cursor` is malformed, not a token the server issued, or was issued under a different `sort`, `view`, `group`, or `read` than the one requested (sort-, view-, group-, and read-scoped); a grouped-feed cursor replayed against the flat feed (or vice versa) fails the same way.                                                                                                                                               |
